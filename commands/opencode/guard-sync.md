...
(contrato del hook y convención de firma) antes de operar. Localiza la skill en
`.agents/skills/project-orchestrator/`, `.claude/skills/project-orchestrator/` del
proyecto, o `~/.claude/skills/project-orchestrator/`; en este documento se abrevia como
`<SKILL>`. Los scripts están en `<SKILL>/scripts/`.

**Portabilidad del hook.** El mecanismo de hooks es propio de cada host. En Claude Code es
`PreToolUse` en `.claude/settings.json` (Paso 3). En otro backend de agentes, cablea el
guard en su equivalente de "antes de escribir un archivo", respetando el mismo contrato:
el evento por stdin como JSON con `tool_name` y `tool_input.file_path` más el contenido
propuesto, y `exit 2` como bloqueo. Si el host **no ofrece** un punto de enganche previo a
la escritura, dilo explícitamente en el reporte y cablea en su lugar el lint de Capa A en
pre-commit: sin uno de los dos, no hay defensa en tiempo de desarrollo.
...
```powershell
& "<SKILL>/scripts/compile-guard-rules.ps1" -ProjectDir "<ruta>"
```
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "pwsh -NoProfile -File \"<SKILL>/scripts/anti-regression-guard.ps1\"",
            "timeout": 60
          }
        ]
      }
...
cmd /c "pwsh -NoProfile -File `"<SKILL>/scripts/anti-regression-guard.ps1`" < `"$env:TEMP\guard-probe.json`" 2>&1"
