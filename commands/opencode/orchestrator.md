...
Carga la skill `project-orchestrator` con la herramienta Skill y ejecútala según el modo
pedido. El mapa completo de modos está en el `COMMANDS.md` de la skill: búscala en
`.agents/skills/project-orchestrator/` o `.claude/skills/project-orchestrator/` del
proyecto, o en `~/.claude/skills/project-orchestrator/` si es global.
...
1. Carga SIEMPRE la skill `project-orchestrator` (herramienta Skill) antes de actuar;
...
3. Si el directorio de trabajo no es un proyecto (es `~` o no hay repo/código),
   detente y pide abrir opencode en el proyecto.
