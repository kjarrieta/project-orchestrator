...
| `/research-file-handling [ruta\|proveedor]` | Investiga la política de manejo de archivos (proyectos propios, doc oficial del proveedor, buenas prácticas) y cierra por entrevista lo que no tenga estándar: parametrizable/fijo, cantidad min/max, extensiones, peso min/max, proveedor. Usado por el Aprendiz (`learner.md`) e Integraciones (`integrations.md`) |
| `/distill-guard <ruta>` | Destila TODO el conocimiento aplicable al stack del proyecto (universal de la skill, empresa, agente-agnóstico, lenguaje/framework, librerías en cascada) en reglas `senal`, en dos fases — D1 identificar cómo está implementado hoy, D2 definir qué se exige — y recompila con `/guard-sync`. **Obligatorio en la corrida inicial de todo proyecto nuevo** (`/orchestrator nuevo`), justo tras fijar el stack y antes de la Fase 4 |
